...
#Class Node: (state, path cost, parent's state) - node với 3 giá trị
    #state          - trạng thái hiện tại
    #path_cost      - chi phí từ start đến goal;
    #parent's state - trạng thái trước đó
class Node:
    ...
    while not frontier.empty():                                       #Chạy đến hết hàng đợi Frontier
        ...
        for child_state in graph[node.state]:                           #For qua các node con của node hiện tại (từ node hiện tại có thể đi tới)
            child_node = Node(child_state,
                              node.path_cost - heuristic(node.state, food) + 1 + heuristic(child_state, food),
                              node)
