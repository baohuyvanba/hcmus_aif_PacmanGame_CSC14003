...
class pacman_game:
    ...
    def pacman_lv1(self):
        ...
        # Run A*
        pathway = bfs.ucs_search(graph_map, pacman_pos, food_pos)
        #
    ...
    def pacman_lv4(self):
        ...
        pacman.pacman_call()
        # Call Food
        # foods = [ag.Food(self, [pos[1],pos[0]]) for pos in list(foods_pos.keys())]
        for foods_pos_t in foods_pos:
            foods_pos_draw_t = [foods_pos_t[1], foods_pos_t[0]]
            food = ag.Food(self, foods_pos_draw_t)
            food.food_display()
        ...
        point = 0
        time_visits = {}
        game_state = ab.GameState(pacman_pos, foods_pos, keys_monster, point, time_visits)
        ...
        '''
        lay ra duoc n trang thai con pacman di (1 buoc)
        '''
        go_home = 0
        catch = 0
        while True:
            ...
            # print(time_visits[best_move])
            # print (game_state.time_visits[game_state.pacman_pos])
            # print (game_state.time_visits[best_move])
